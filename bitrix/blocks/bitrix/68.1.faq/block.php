<section class="landing-block g-pa-30">
	<div class="container">
		<!-- card 1 -->
		<div class="landing-block-card">
			<h3
				class="landing-block-faq-visible g-cursor-pointer g-color g-color--hover"
				style="--color:hsla(0, 0%, 43%, 1); --color-hover:hsla(0, 0%, 0%, 1);">
				1 What is Sites & stores?
			</h3>
			<div class="landing-block-faq-hidden">
				<div class="g-pa-20">
					A template-based website builder allowing you to create beautiful websites, landing pages, and online stores.
				</div>
			</div>
		</div>
		<!-- card 2 -->
		<div class="landing-block-card">
			<h3
				class="landing-block-faq-visible g-cursor-pointer g-color g-color--hover"
				style="--color:hsla(0, 0%, 43%, 1); --color-hover:hsla(0, 0%, 0%, 1);">
				2 What is Communications?
			</h3>
			<div class="landing-block-faq-hidden">
				<div class="g-pa-20">
					Virtual office for your company to work, communicate and collaborate - at home or in the office.
				</div>
			</div>
		</div>
		<!-- card 3 -->
		<div class="landing-block-card">
			<h3
				class="landing-block-faq-visible g-cursor-pointer g-color g-color--hover"
				style="--color:hsla(0, 0%, 43%, 1); --color-hover:hsla(0, 0%, 0%, 1);">
				3 What is CRM?
			</h3>
			<div class="landing-block-faq-hidden">
				<div class="g-pa-20">
					Packed with client management, customer support, and marketing automation features, Bitrix24 CRM is a digital sales suite.
				</div>
			</div>
		</div>
		<!-- card 4 -->
		<div class="landing-block-card">
			<h3
				class="landing-block-faq-visible g-cursor-pointer g-color g-color--hover"
				style="--color:hsla(0, 0%, 43%, 1); --color-hover:hsla(0, 0%, 0%, 1);">
				4 What is Contact center?
			</h3>
			<div class="landing-block-faq-hidden">
				<div class="g-pa-20">
					Any and every tool you may need for quick and efficient communication with your clients.
				</div>
			</div>
		</div>
		<!-- card 5 -->
		<div class="landing-block-card">
			<h3
				class="landing-block-faq-visible g-cursor-pointer g-color g-color--hover"
				style="--color:hsla(0, 0%, 43%, 1); --color-hover:hsla(0, 0%, 0%, 1);">
				5 What is Tasks & projects?
			</h3>
			<div class="landing-block-faq-hidden">
				<div class="g-pa-20">
					Set tasks, receive work reports, and manage projects online with the ultimate "get things done" tool from Bitrix24.
				</div>
			</div>
		</div>
	</div>
</section>